...
public:
  typedef Priorielem<T> tipoelem;

  CodaP ();         //COSTRUTTORE
  CodaP (int);      //COSTRUTTORE CON DIMENSIONE
  CodaP(CodaP<T>&); //COSTRUTTORE DI COPIA
  ~CodaP ();        //DISTRUTTORE
  ...
  //SOVRACCARICO OPERATORI
	CodaP<T>& operator=(CodaP<T> &);   //SOVRACCARICO OPERATORE ASSEGNAZIONE =
	bool operator==(CodaP<T> &);      //SOVRACCARICO OPERATORE UGUAGLIANZA ==
...
template<class T>
CodaP<T>::CodaP(CodaP<T>& l){
  ultimo = 0;
  heap = new tipoelem[l.MAXLUNG];
...
template<class T>
CodaP<T>& CodaP<T>::operator=(CodaP<T>& l){
  MAXLUNG=l.MAXLUNG;
  ultimo = 0;
  heap = new tipoelem[l.MAXLUNG];
...
//SOVRACCARICO OPERATORE ==
template<class T>
bool CodaP<T>::operator==(CodaP<T> &l){	
  bool flag=true;

    if(insiemeVuoto() && l.insiemeVuoto())
        return true;

    if(!l.insiemeVuoto() && !insiemeVuoto()) {
	  	  tipoelem el = l.min();
        tipoelem e = this->min();
