...
class Dizionario : private Coppia<K, V>{
...
private:
    ...
    unsigned int hash(string str);                  //CALCOLA LA POSIZIONE NELLA TABELLA DELLA CHIAVE ( % dim )
    unsigned int H(const K& chiave);                //FUNZIONE DI HASH
};
...
bool Dizionario<K,V>::appartiene(const chiave& key) {
    bool trovato = false;
    posizione iter; 
    unsigned int pos = H(key); //Funzione hash sullla chiave per trovare la posizione 
...
//CALCOLA LA POSIZIONE NELLA TABELLA DELLA CHIAVE ( % dim )
template<typename K, typename V>
unsigned int Dizionario<K, V>::H(const K& chiave) {
    return (hash(chiave) % dim);
}

//FUNZIONE DI HASH
template<typename K, typename V>
unsigned int Dizionario<K, V>::hash(string str) {
