...
//Classe matgraph (NON ORIENTATO) di tipo MATRICE DI ADIACENZA ESTESA con Mtrice NxN (peso e vuoto), Etichetta, Vuoto
...
public:
    ...
    void insArch(node, node, weight);   //Inserisce un arco tra i due nodi e il peso
    void eraseNode(node);               //Elimina un nodo dalla matrice di adiacenza  (non ci devono essere archi)
    void eraseArch(node, node);         //Elimina un arco della matrice di adiacenza      
    ...
    //funzione che restituisce il numero di archi del nodo a cui si applica
    int outDegree(node) const;
    //metodo che  restituisce la media del numero di archi per ogni nodo del grafo
...
//Inserisce un arco tra i due nodi e il peso
template <class E, class P>
void matgraph<E, P>::insArch(node n1, node n2, weight w)
{
    ...
    matrix[n2.getId()].arches[n1.getId()].weight = w;   
    matrix[n2.getId()].arches[n1.getId()].empty = false;

    arches++;
}

//Elimina un nodo dalla matrice di adiacenza (non ci devono essere archi)
template <class E, class P>
...
//Elimina un arco della matrice di adiacenza  
template <class E, class P>
void matgraph<E, P>::eraseArch(node n1, node n2)
{
    if(isArch(n1, n2)) {
        ...
        matrix[n2.getId()].arches[n1.getId()].empty = true; //arco vuoto
        matrix[n2.getId()].arches[n1.getId()].weight = 0;   //imposto il peso ad un valore nullo
